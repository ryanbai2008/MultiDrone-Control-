...
def localize():
    ...
    while running:
        ...
        if rotating2:
            ...
            yaw2 = map2.get_angle(drone2current_pos, personpospx, (drone2current_pos[0], drone2current_pos[1]+10))
        ...
        # inside your loop
        clock.tick(int(1 / updateTime)/8)  # updates per second
...
frame_read = drone1.get_frame_read()
...
out2 = cv2.VideoWriter("output2.avi", fourcc, 30, (frame_width, frame_height))

def tracking_loop(drone, cv, drone_id, output):
    frame = drone.get_frame_read().frame  # Get latest video frame
    turn, detections = cv.center_subject(frame, drone_number=drone_id)
    ...
    if frame is None or frame.size == 0:
        logging.error(f"No frame received from Drone {drone_id}")
        return False
    # Write frame
    ...
    if turn == 0:
        turn = 25
        facing_human = False
    elif turn == 1:  # Human centered
        facing_human = True
        turn = 0
    drone.send_rc(0, 0, 0, turn)
    output.write(frame)
    return facing_human


def movement_loop(drone, cv, drone_id, output):
    frame = drone.get_frame_read().frame  # Get latest video frame
    turn, detections = cv.center_subject(frame, drone_number=drone_id)
    for det in detections:
            x, y, w, h = det["box"]
...
try:
    ...
    #turn on drone and get to height
    ...
    while (not facing_human_1) or (not facing_human_2):
        ...
        if (img1 is not None) and (img2 is not None):
            facing_human_1 = tracking_loop(drone1, drone_1_CV, 1, out)
            facing_human_2 = tracking_loop(drone2, drone_2_CV, 2, out2)
        else:
    ...
    while total_time < 60:
        ...
        if (img1 is not None) and (img2 is not None):
            ...
            turn_1 = movement_loop(drone1, drone_1_CV, 1, out)
            turn_2 = movement_loop(drone2, drone_2_CV, 2, out2)
